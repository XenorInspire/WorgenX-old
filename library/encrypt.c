/* This part was written by noryb009 */
/* The github repository : https://github.com/noryb009/sha256 */
/* More info about sha : http://csrc.nist.gov/publications/fips/fips180-2/fips180-2.pdf */
...
void hex(void *buffer, size_t len, char **finalHash)
{
    for (size_t i = 0; i < len; i++)
    {
        ...
        sprintf(temp, "%02x", ((char *)buffer)[i] & 0xff);
...
char * mainEncrypt(char *plainText)
{
    ...
    char *finalHash = malloc(SIZE_SHA256 * sizeof(char));
    checkPtr(msgPad);
    checkPtr(finalHash);

    memcpy(msgPad, plainText, len);
    msgPad[len] = 0x80;
    l = swapE64(l);
